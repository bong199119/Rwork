...
#귀무가설 : 교육수준과 흡연율간에는 관련성이 없다.
#대립가설 : 교육수준과 흡연율간에는 관련성이 있다.
...
#<단계 5> 검정결과 해석
# X-squared = 18.911, df = 4, p-value = 0.0008183 0.05보다 작음 -> 귀무가설 기각
...
#[단계 4] 독립성 검정
CrossTable(data$position, data$age3, chisq = T)
# Chi^2 =  287.8957     d.f. =  8     p =  1.548058e-57 


chisq.test(data$position, data$age3)
#X-squared = 287.9, df = 8, p-value < 2.2e-16

#[단계 5] 검정결과 해석
# p-value < 2.2e-16 < 0.05 이므로 나이와 직위는 관련성이 있다.
...
#[단계 5] 검정결과 해석
# p-value = 6.901e-12 < 0.05 직업유형에 따른 응답정도에 차이가 있다.
